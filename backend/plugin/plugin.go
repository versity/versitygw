...
// Since no options are passed to init the plugin, it is expected that the
// plugin will handle its own configuration and initialization.
...
func NewPluginBackend(path string) (*PluginBackend, error) {
	...
	if !ok {
		return nil, fmt.Errorf("InitPlugin symbol is not a func() error")
	}
