...
func readDir(path string, walkstate *walkState) {
	if err := walkstate.ctx.Err(); err != nil {
...
func quickWalk(walkstate *walkState) error {
	...
	if strings.Contains(walkstate.prefix, pathSeparator) {
		if idx := strings.LastIndex(walkstate.prefix, pathSeparator); idx > 0 {
			...
			// reuse the ReadDir result; if successful and non-empty we start
			// from prefixDir directly, avoiding a second ReadDir call later.
			if entries, err := fs.ReadDir(walkstate.fileSystem, prefixDir); err == nil && len(entries) > 0 {
				rootDir = prefixDir
			}
		}
	}
