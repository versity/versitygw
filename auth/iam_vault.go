...
func NewVaultIAMService(rootAcc Account, endpoint, secretStoragePath, authMountPath, kvMountPath, rootToken, roleID, roleSecret, serverCert, clientCert, clientCertKey string) (IAMService, error) {
	...
	authReqOpts := []vault.RequestOption{}
	// if auth mount path is not specified, it defaults to "approle"
	if authMountPath != "" {
		authReqOpts = append(authReqOpts, vault.WithMountPath(authMountPath))
	}

	kvReqOpts := []vault.RequestOption{}
	// if kv mount path is not specified, it defaults to "kv-v2"
	if kvMountPath != "" {
		kvReqOpts = append(authReqOpts, vault.WithMountPath(kvMountPath))
	}
