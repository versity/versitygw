...
func (cr *CORSRule) Validate() error {
	allowedHeadersLen, allowedMethodsLen, exposedHeadersLen := len(cr.AllowedHeaders), len(cr.AllowedMethods), len(cr.ExposeHeaders)
	loopLimit := max(allowedHeadersLen, allowedMethodsLen, exposedHeadersLen)

	for i := range loopLimit {
		// validate cors allowed headers
		if i < allowedHeadersLen {
			header := cr.AllowedHeaders[i]
			if !header.IsValid() {
		...
		if i < allowedMethodsLen {
			method := cr.AllowedMethods[i]
			if !method.IsValid() {
		...
		// validate cors exposed headers
		if i < exposedHeadersLen {
			header := cr.ExposeHeaders[i]
			if !header.IsValid() {
