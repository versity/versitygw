...
func TestBucketPolicyDecision_Condition(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			decision, err := verifyBucketPolicy([]byte(tt.policy), Account{Access: "someaccess"}, "mybucket", tt.object, tt.condCtx, nil, tt.action)
...
func TestBucketPolicyDecision_UnevaluableConditionFailsClosed(t *testing.T) {
	...
	decision, err := verifyBucketPolicy([]byte(policy), Account{Access: "someaccess"}, "mybucket", "key", nil, nil, GetObjectAction)
	assert.NoError(t, err)
