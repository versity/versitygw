...
func TestMarshalStructToXML(t *testing.T) {
	type args struct {
		data interface{}
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := MarshalStructToXML(tt.args.data)
			if (err != nil) != tt.wantErr {
				t.Errorf("MarshalStructToXML() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("MarshalStructToXML() got = %v, want %v", got, tt.want)
			}
		})
	}
}
