...
func parseCreateBucketPayload(input string) ([]byte, error) {
	...
	inputParts := splitTopLevel(input)
...
// parseTagging parses a tag set expressed in shorthand syntax into AWS CLI tags.// Expected format:
//
...
func parseTagging(input string) ([]types.Tag, error) {
	...
	tagComponents := splitTopLevel(input)
