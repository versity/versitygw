...
@test "REST - ListObjects - Marker returned when request has no marker" {
  ...
  run list_objects_check_empty_echo_element "$bucket_name" "FALSE" "Marker"
  assert_success
}

# tags: curl,ListObjectsV2,continuation-token
@test "REST - ListObjectsV2 - ContinuationToken returned when request has no token" {
  run get_bucket_name "$BUCKET_ONE_NAME"
  assert_success
  bucket_name="$output"
  ...
  run list_objects_check_empty_echo_element "$bucket_name" "TRUE" "ContinuationToken"
  assert_success
}
