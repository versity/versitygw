...
get_object_rest_expect_error() {
  if ! check_param_count_v2 "bucket name, object key, output file, response code, error, message" 6 $#; then
    return 1
  fi
  env_vars="BUCKET_NAME=$1 OBJECT_KEY=$2 OUTPUT_FILE=$3"
  if ! send_rest_command_expect_error "$env_vars" "./tests/rest_scripts/get_object.sh" "$4" "$5" "$6"; then
    log 2 "error sending REST command and checking error"
    return 1
  fi
  return 0
...
get_object_rest_with_user() {
  if ! check_param_count_v2 "username, password, bucket name, object key, output file" 3 $#; then
    return 1
  fi
