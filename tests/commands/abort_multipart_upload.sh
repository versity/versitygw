...
abort_multipart_upload_with_user() {
  ...
  record_command "abort-multipart-upload" "client:s3api"
  if ! abort_multipart_upload_error=$(send_command AWS_ACCESS_KEY_ID="$4" AWS_SECRET_ACCESS_KEY="$5" aws --no-verify-ssl s3api abort-multipart-upload --bucket "$1" --key "$2" --upload-id "$3" 2>&1); then
