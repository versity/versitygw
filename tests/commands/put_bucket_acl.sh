...
put_bucket_canned_acl_s3cmd() {
  ...
  if [[ $# -ne 3 ]]; then
    log 2 "put bucket acl command requires bucket name, permission, username"
    return 1
  fi
  if ! error=$(s3cmd "${S3CMD_OPTS[@]}" --no-check-certificate setacl "s3://$1" --acl-grant="$2":"$3" 2>&1); then
