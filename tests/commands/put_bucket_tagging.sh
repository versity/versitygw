...
put_bucket_tagging_with_user() {
  ...
  assert [ $# -eq 5 ]
  record_command "put-bucket-tagging" "client:$1"
  if ! error=$(send_command AWS_ACCESS_KEY_ID="$4" AWS_SECRET_ACCESS_KEY="$5" aws --no-verify-ssl s3api put-bucket-tagging --bucket "$1" --tagging "TagSet=[{Key=$2,Value=$3}]"); then
