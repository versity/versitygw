...
func GetObject_not_enabled_checksum_mode(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		_, err := putObjectWithData(350, &s3.PutObjectInput{
			Bucket:            &bucket,
			Key:               &obj,
			ChecksumAlgorithm: types.ChecksumAlgorithmSha1,
		}, s3client)
...
func ListObjects_with_checksum(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		checksumAlgos := types.ChecksumAlgorithmCrc32.Values()
		checksumAlgos = append(checksumAlgos, "")

		for i, el := range checksumAlgos {
...
func UploadPart_required_checksum(s *S3Conf) error {
	testName := "UploadPart_required_checksum"
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		obj := "my-obj"
		mp, err := createMp(s3client, bucket, obj, withChecksum(types.ChecksumAlgorithmCrc32c))
		if err != nil {
		...
		cancel()
		if err := checkApiErr(err, s3err.GetChecksumTypeMismatchErr(types.ChecksumAlgorithmCrc32c, "null")); err != nil {
			return err
		}
...
func UploadPart_null_checksum(s *S3Conf) error {
	testName := "UploadPart_null_checksum"
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		obj := "my-obj"
		mp, err := createMp(s3client, bucket, obj)
		if err != nil {
		...
		cancel()
		if err := checkApiErr(err, s3err.GetChecksumTypeMismatchErr("null", types.ChecksumAlgorithmCrc32)); err != nil {
			return err
		}
...
func ListMultipartUploads_with_checksums(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		for _, el := range []struct {
			...
			key := el.obj
			mp, err := createMp(s3client, bucket, key, withChecksum(el.algo))
			if err != nil {
				return err
			}
...
func CompleteMultipartUpload_invalid_checksum_part(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		parts, _, err := uploadParts(s3client, 5*1024*1024, 3, bucket, obj, *mp.UploadId, withChecksum(types.ChecksumAlgorithmCrc32))
		if err != nil {
			return err
		}
...
func CompleteMultipartUpload_incorrect_checksum_part(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		parts, _, err := uploadParts(s3client, 5*1024*1024, 3, bucket, obj, *mp.UploadId, withChecksum(types.ChecksumAlgorithmSha256))
		if err != nil {
			return err
		}
...
func CompleteMultipartUpload_different_checksum_part(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		parts, _, err := uploadParts(s3client, 5*1024*1024, 3, bucket, obj, *mp.UploadId, withChecksum(types.ChecksumAlgorithmCrc32c))
		if err != nil {
			return err
		}
...
func CompleteMultipartUpload_missing_part_checksum(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		parts, _, err := uploadParts(s3client, 5*1024*1024, 3, bucket, obj, *mp.UploadId, withChecksum(types.ChecksumAlgorithmSha1))
		if err != nil {
			return err
		}
...
func CompleteMultipartUpload_unnecessary_checksum_part(s *S3Conf) error {
	testName := "CompleteMultipartUpload_unnecessary_checksum_part"
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		obj := "my-obj"
		mp, err := createMp(s3client, bucket, obj)
		if err != nil {
		...
		for i, el := range parts {
			...
			if i == 1 {
				cParts[0].ChecksumCRC32 = getPtr("sGc9Hg==")
			}
		}
...
func CompleteMultipartUpload_invalid_final_checksums(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		for _, algo := range types.ChecksumAlgorithmCrc32.Values() {
			...
			parts, _, err := uploadParts(s3client, 15*1024*1024, 3, bucket, obj, *mp.UploadId, withChecksum(algo))
			if err != nil {
				return err
			}
			...
			cancel()
			if err := checkApiErr(err, s3err.GetInvalidChecksumHeaderErr(fmt.Sprintf("x-amz-checksum-%v", strings.ToLower(string(algo))))); err != nil {
				return err
			}
		}
...
func CompleteMultipartUpload_incorrect_final_checksums(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		for _, algo := range types.ChecksumAlgorithmCrc32.Values() {
			...
			parts, _, err := uploadParts(s3client, 5*1024*1024, 3, bucket, obj, *mp.UploadId, withChecksum(algo))
			if err != nil {
				return err
			}
...
func CompleteMultipartUpload_should_calculate_the_final_checksum(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		for _, algo := range types.ChecksumAlgorithmCrc32.Values() {
			...
			parts, csum, err := uploadParts(s3client, 15*1024*1024, 3, bucket, obj, *mp.UploadId, withChecksum(algo))
			if err != nil {
				return err
			}
			...
			case types.ChecksumAlgorithmSha1:
				if getString(res.ChecksumSHA1) != csum {
					return fmt.Errorf("expected the final sha1 checksum to be %v, instead got %v", csum, getString(res.ChecksumSHA1))
				}
			case types.ChecksumAlgorithmSha256:
				if getString(res.ChecksumSHA256) != csum {
					return fmt.Errorf("expected the final sha256 checksum to be %v, instead got %v", csum, getString(res.ChecksumSHA256))
				}
			}
		}
...
func CompleteMultipartUpload_should_verify_the_final_checksum(s *S3Conf) error {
	...
	return actionHandler(s, testName, func(s3client *s3.Client, bucket string) error {
		...
		for _, algo := range types.ChecksumAlgorithmCrc32.Values() {
			...
			case types.ChecksumAlgorithmSha1:
				mpInput.ChecksumSHA1 = &csum
			case types.ChecksumAlgorithmSha256:
				mpInput.ChecksumSHA256 = &csum
			}
